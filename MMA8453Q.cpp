/*
  ...
  Foundation, Inc., 51 Franklin St, Fifth Floor, Boston, MA  02110-1301  USA
    ...
    Version 1.0 LucidTronix added get x, y, and z functions
...
int MMA8453Q::getX(bool isHighRes)
{
  int x;
  byte buf[2];

  if (isHighRes) {
    regRead(REG_OUT_X_MSB, buf, 2);
    x = buf[0] << 2 | buf[1] >> 6 & 0x3;
  }
  else {
    regRead(REG_OUT_X_MSB, buf, 1);
    x = buf[0] << 2;
  }
  if (x > 511) x -= 1024;
  return x;
...
int MMA8453Q::getY(bool isHighRes)
{
  int y;
  byte buf[2];

  if (isHighRes) {
    regRead(REG_OUT_Y_MSB, buf, 2);
    y = buf[0] << 2 | buf[1] >> 6 & 0x3;
  }
  else {
    regRead(REG_OUT_Y_MSB, buf, 1);
    y = buf[0] << 2;
  }
  if (y > 511) y -= 1024;
  return y;
...
int MMA8453Q::getZ(bool isHighRes)
{
  int z;
  byte buf[2];

  if (isHighRes) {
    regRead(REG_OUT_Z_MSB, buf, 2);
    z = buf[0] << 2 | buf[1] >> 6 & 0x3;
  }
  else {
    regRead(REG_OUT_Z_MSB, buf, 1);
    z = buf[0] << 2;
  }
  if (z > 511) z -= 1024;
  return z;
